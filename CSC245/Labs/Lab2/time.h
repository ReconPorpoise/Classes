...
public:
    ...
    // appends AM or PM to the time as well as prints the 24 hour time as 12 hour time
    void WriteAmPm() const;

    // deconstructs any Time objects when the program is done running
    ~Time();

    // tells you if it's time for x meal depending on the time
    void Mealtime() const;
    Time( /* in */ int initHrs,
              /* in */ int initMins,
              /* in */ int initSecs );
