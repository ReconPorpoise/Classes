...
public class Search { 
 public static void main(String argv[]) throws IOException { 
  ...
  private static boolean search(Node head, Comparable key)
  {
     ...
     //         list
	// set the first index as head
	Node curr = head;
	boolean found = false;
	
	// while there are still nodes...
	while (curr != null) {
		// if the current item and the key are the same set found to truek
		if (((Comparable) curr.getItem()).compareTo(key) == 0) {
			found = true;
		}
		// set current item to be the next node value in the list
		curr = curr.getNext();
	}
	return found;
  } 
